...
class APIService {
    ...
    if (appState.isDemoMode || !appState.apiUrl) {
        // Demo mode - simulate delay and save to localStorage
        await new Promise(resolve => setTimeout(resolve, 800));
        
        // Simulate API response
        return { 
            success: true, 
            message: `Data saved locally (Demo Mode).`,
            endpoint: endpoint,
            data: data
        };
    }
    ...
    try {
        console.log(`Sending ${endpoint} data:`, data);
        ...
        UIUtils.showToast(`⚠️ API connection failed, saving locally`, 'warning');
        ...
        return { 
            success: true, 
            message: 'Saved locally (API backup mode)',
            isBackup: true
        };
    }
}
    ...
    if (!apiUrl) {
        return { 
            success: false, 
            message: 'No API URL configured. Please enter your Google Apps Script URL.' 
        };
    }

    // Show loading toast
    UIUtils.showToast('Testing connection to Google Sheets...', 'info');
    
    try {
        console.log('Testing API URL:', apiUrl);
        
        // Test with GET request first (simpler)
        ...
        if (result && result.success === true) {
            return { 
                success: true, 
                message: '✅ Connection successful! Google Sheets API is ready.',
                details: result
            };
        } else {
        ...
        if (error.message.includes('Failed to fetch') || error.message.includes('CORS')) {
            errorMessage += '\n\n🔧 Try these fixes:';
            errorMessage += '\n1. Open the URL directly in browser first';
        ...
        return { 
            ...
            urlTest: `${apiUrl}?action=test`  // URL to test manually
        };
    }
}
}
